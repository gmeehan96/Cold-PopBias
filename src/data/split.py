...
parser.add_argument(
    "--datadir", type=str, default="./", help="Director of the dataset."
)
parser.add_argument(
    "--warm_ratio", type=float, default=0.8, help="Warm ratio of all items."
)
...
print(
    f"Global sparse rate: %.4f"
    % ((user_num * item_num - new_len) / (user_num * item_num) * 100.0)
)
...
print(
    "warm\t{}\t{}\t{}".format(
        len(set(df_warm["user"])), len(set(df_warm["item"])), df_warm.shape[0]
    )
)
print(
    "cold\t{}\t{}\t{}".format(
        len(set(df_cold["user"])), len(set(df_cold["item"])), df_cold.shape[0]
    )
)
...
warm_val_idx = np.array(
    list(set(warm_val_idx.tolist()) - set(idx_to_move.tolist())), dtype=np.int64
)
...
warm_test_idx = np.array(
    list(set(warm_test_idx.tolist()) - set(idx_to_move.tolist())), dtype=np.int64
)
warm_train_idx = np.concatenate([warm_train_idx, idx_to_move], axis=0)
...
print(
    "Warm train splitting finished: {} -> {}".format(
        org_warm_train_len, len(warm_train_idx)
    )
)
...
print(
    "train\t%d\t%d\t%d"
    % (
        len(set(df_warm_train["user"])),
        len(set(df_warm_train["item"])),
        len(warm_train_idx),
    )
)
print(
...
print(
    "test\t%d\t%d\t%d"
    % (
        len(set(df_warm_test["user"])),
        len(set(df_warm_test["item"])),
        len(warm_test_idx),
    )
)
...
df_cold_val.to_csv(
    os.path.join(output_path, f"cold_{args.cold_object}_val.csv"), index=False
)
df_cold_test.to_csv(
    os.path.join(output_path, f"cold_{args.cold_object}_test.csv"), index=False
)
...
print(
    "test\t%d\t%d\t%d"
    % (
        len(set(df_cold_test["user"])),
        len(set(df_cold_test["item"])),
        len(cold_test_idx),
    )
)
