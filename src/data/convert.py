...
class Timer(object):
    ...
    def logging(self, message):
        ...
        print(
            "{} {} {:.0f}s {:.0f}s | {}".format(
                current_time, self._name, self.run_time, self.stage_time, message
            )
        )
...
df_cold_test = pd.read_csv(
    os.path.join(store_path, f"cold_{args.cold_object}_test.csv")
)
print(
    df_emb.shape,
    df_warm_val.shape,
...
procedure_timer.logging(
    "Convert {} successfully, store the dict to {}".format(args.dataset, dict_path)
)
