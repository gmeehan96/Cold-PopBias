...
class GoRec_Learner(nn.Module):
    ...
    def clustering(self):
        ...
        cluster_fit = KMeans(n_clusters=self.args.pre_cluster_num, random_state=0).fit(
            self.item_content.cpu().detach().numpy()
        )
        ...
        for i in range(self.args.pre_cluster_num):
            cluster_cfmean.append(
                self.embedding_dict[self.args.cold_object + "_emb"][
                    np.where(
                        self.cluster_label[: len(self.data.mapped_warm_item_idx)] == i
                    )
                ].mean(0)
            )
